...
# Improve labeling for x and y axis and adapt color scheme for dark theme
...
# Improve labeling for y axis and adapt color scheme for dark theme
...
# Create labels with total and percentage
...
# Combine both lists with 'SQL JOIN'
...
# Create a comparable dataset with IMR and HDI using the 'combine' function from the R package 'ourdata'
df <- combine(imr$name, hdi$country, imr$value, hdi$hdi, col1 = "Country", col2 = "IMR", col3 = "HDI")
...
# Create frequency plot for the top 20 items
...
# Show the top 5 rules (only 2 decimal places)
options(digits=2)
inspect(rules[1:5])
...
# Load package with visualization association rules
library(arulesViz)
...
# Create dataset of crime in the USA
...
# Create plot and regression line
...
Since my childhood I have been playing **computer games** of all genres. In particular **adventure games**, such as the classics *Indiana Jones*, *Monkey Island* and the modern offshoot *Broken Sword*, have always lured me to the screen. So it was not far-fetched to create my own adventure game.

Here is my [**BenBox.org**](https://www.benbox.org) website for my *iPad* **game prototype**, which I created during my studies at **Fresenius University of Applied Sciences** in the **Health & Management (B. Sc.)** program:

```{r showBenBox.org, echo = FALSE, fig.cap = "Fig. 2.3.9a: BenBox.org Website"}
