...
```{r BasketAnalyse, fig.height=4, fig.showtext = TRUE, results = 'hold', echo = TRUE,  eval = FALSE}
# Bibliotheken laden
library(arules)     # Paket mit Mining Datensätzen und Assoziationsregeln
library(arulesViz)  # Paket mit Visualisations-Assoziationsregeln
...
# Daten einlesen
data <- read.csv(system.file("extdata", "cereals.csv", package = "ourdata", mustWork = FALSE), header=T)
print(data)
...
```{python pDeepL, fig.height=4, fig.showtext = TRUE, results = 'hold', class.source = "SCodeP", class.output = "OCodeP"}
...
```{python DeepLPythonCode, code = readLines(system.file("extdata", "py_deepl.py", package = "ourdata", mustWork = FALSE)), , class.source = "SCodeP", class.output = "OCodeP"}
```
...
```{r Abb2.3.8a, echo = FALSE, fig.cap = "Abb. 2.3.8a: Big Data Jobs", out.width = '100%'}
